...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
  ...
  switch(ip->ip_p) {
    ...
    reply_to_icmp(ip->ip_src, ip->ip_dst);
    return;
    case IPPROTO_IP:
    printf("   Protocol: IP\n");
    return;
...
int reply_to_icmp(struct in_addr src, struct in_addr dst)
{
  int s = socket (PF_INET, SOCK_RAW, IPPROTO_ICMP);  /* open raw socket */

  const char *src_c = inet_ntoa(src);
  const char *dst_c = inet_ntoa(dst);
  ...
  struct sockaddr_in sin;
    ...
    sin.sin_addr.s_addr = inet_addr (dst_c);
    ...
    iph.ip_sum = 0x0;
    iph.ip_src.s_addr = inet_addr(src_c);
    iph.ip_dst.s_addr = inet_addr(dst_c);
    iph.ip_sum = 0;
...
int main(int argc, char **argv)
{
  ...
  if (handle == NULL) {
    handle = pcap_open_offline(argv[1], errbuf);  // Checking the user gave a .pcap file.
    if (handle == NULL)
    {
